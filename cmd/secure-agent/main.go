...
func printUsage() {
	...
	fmt.Println("  secure-agent cost [--since D] [--by G] [--tz M]   Model-call spend by repo|branch|harness|session|model|provider|day (--json for the raw body)")
	fmt.Println("                                           --by provider: recorded provider, else the model's vendor; --by day: local days, oldest first")
	fmt.Println("                                           --tz: minutes east of UTC for --by day (default: this machine's offset)")
	...
	fmt.Println("  secure-agent worktrees ask <path>        Resume the Claude Code or Codex session that worked in a keep/review worktree: open a PR for its work, or say it can go")
