...
### Added
- Events rows name every trace kind (TOOL, TURN, MODEL) and file, exec and connection kind with its tool, model, tokens, cost or price class, path or host; a pid-0 row names its session; rows sort newest first and show the date when not from today; `/events` model calls carry `price_class`; a Hermes or openclaw database with no saved watermark starts at its last 24 h, not message 0.
